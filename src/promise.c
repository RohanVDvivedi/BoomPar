...
int set_promised_result(promise* p, void* res)
{
	...
	promise_completion_callback* pcc_to_execute = NULL;
	...
	if(!p->output_result_ready)
	{
		...
		// if the output is set, then we also set the promise_completion_callback that we will have to run
		pcc_to_execute = p->promised_callback;
	}
...
int set_promised_callback(promise* p, promise_completion_callback* promised_callback)
{
	...
	if(p->promised_callback == NULL)
	{
		p->promised_callback = promised_callback;
	...
	if(is_result_ready && was_promise_completion_callback_set)
		promised_callback->callback_function(p, promised_callback->callback_param);
