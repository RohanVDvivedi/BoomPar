...
// Note: single shot implies, running once and then it will either pause OR wait for the period_in_microsecond to elapse
...
enum periodic_job_state
{
	...
	PJ_RUNNING, // job is in running state
	...
	PJ_SHUTDOWN, // job is in shutdown state
};
...
struct periodic_job
{
	...
	// wait for the periodic job itself (internal), to wait for its PJ_PAUSED and PJ_WAITING state
	...
	// 1 bit each for SHUTDOWN_CALLED, PAUSE_CALLED, RESUME_CALLED, and SINGLE_SHOT_CALLED, (in decreasing order of their priority)
	int event_vector;

	pthread_cond_t stop_wait; // wait on this condition variable to wait for the job to change state to PJ_PAUSED or PJ_SHUTDOWN
};

// for any of the functions below, the period in microseconds can never be BLOCKING (wait forever) or NON_BLOCKING (wait never)
...
// remember the periodic_job starts in PJ_PAUSED state, you need to resume it after initialization
...
// returns instantaneous value of period_in_microseconds representing the period of the periodic_job
uint64_t get_period_for_periodic_job(periodic_job* pjob);

// below 4 are the events that you send to the periodic job to make it transition into different states
...
// tries to immediately transition a PJ_PAUSED or PJ_WAITING periodic job into PJ_SINGLE_SHOT_ON_PAUSED or PJ_SINGLE_SHOT_ON_WAITING states, respectively
// it allows the periodic job to quit waiting and attempt to run the periodic_job_function just once, then it is again transitioned back into its prior state
...
// keeps you waiting forever until the periodic_job does not reach PJ_PAUSED or PJ_SHUTDOWN state
